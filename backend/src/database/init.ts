...
const createTables = async () => {
  ...
  try {
    ...
    await connection.query(brokersTable);
    ...
    await connection.query(propertiesTable);
    ...
    if (Array.isArray(existingAdmin) && existingAdmin.length === 0) {
      ...
      const password_hash = await bcrypt.hash(adminPassword, 8);
      await connection.query('INSERT INTO admins (name, email, password_hash) VALUES (?, ?, ?)', ['Admin Padrão', adminEmail, password_hash]);
